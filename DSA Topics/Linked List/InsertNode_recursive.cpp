...
{
     // Base case: if pos is 0, insert the newNode at the beginning
    if (pos == 0) {
        newNode->next = head;
        return newNode;
    }

    // If the list is empty and position is not 0, return the head
    if (!head) return head;

    // Recursive step: move to the next node and decrease the position by 1
