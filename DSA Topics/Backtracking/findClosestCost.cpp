...
void findClosestCost(int currentCost, int index, vector<int>& toppingCosts, int target, int& closest) {
    // Base case: If all toppings are considered
    if (index == toppingCosts.size()) {
        // Update closest cost if this is a better option
    ...
    // Recursively explore 3 options: 0, 1, or 2 of the current topping
    findClosestCost(currentCost, index + 1, toppingCosts, target, closest); // 0 of this topping
    findClosestCost(currentCost + toppingCosts[index], index + 1, toppingCosts, target, closest); // 1 of this topping
    findClosestCost(currentCost + 2 * toppingCosts[index], index + 1, toppingCosts, target, closest); // 2 of this topping
}
...
int closestCost(int n, int m, vector<int> &baseCosts, vector<int> &toppingCosts, int target) {
    ...
    for (int base : baseCosts) {
        // Start with each base cost
        findClosestCost(base, 0, toppingCosts, target, closest);
    }
