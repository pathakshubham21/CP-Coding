...
pair<int, int> predecessorSuccessor(TreeNode *root, int key) {
    pair<int, int> result;
    result.first = -1;  // To store the predecessor
    result.second = -1; // To store the successor
    ...
    while (current != NULL) {
        if (current->data < key) {
            result.first = current->data; // Update predecessor
    ...
    while (current != NULL) {
        if (current->data > key) {
            result.second = current->data; // Update successor
            current = current->left;
        } else {
            current = current->right;
        }
